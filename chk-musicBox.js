...
async function validateMusicBox(opt = { action: '', dirPath: '' }) {
  /** 取得來源資料 */
  ...
  console.log(`Start checking music box ${MUSIC_BOX_CATEGORIES}`);

  const myFailLog = `${opt.action}_musicBoxLog.txt`;
  const fails = [];
  if (fs.existsSync(myFailLog)) {
    await fs.unlink(myFailLog);
  }
  // Check category info
  for (const cid of MUSIC_BOX_CATEGORIES) {
    ...
    const validAlbums = currCategory.albums.map(x => MUSIC_BOX_ALBUMS.find(y => y === x)).filter(z => !!z);
